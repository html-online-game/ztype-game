...
ig.baked=true;ig.module('game.menus.title').requires('game.menus.base','game.ease').defines(function(){MenuItemNormalMode=MenuItem.extend({getText:function(){return'new game';},ok:function(){ig.game.setGame();},});MenuItemPauseMenu=MenuItem.extend({getText:function(){return'sound menu';},ok:function(){ig.game.menu=new MenuPause();},});MenuItemRemoveAds=MenuItem.extend({getText:function(){return'remove ads';},ok:function(){ig.game.purchaseRemoveAds();}});MenuItemGameCenter=MenuItem.extend({getText:function(){return'gamecenter';},ok:function(){ig.game.gameCenter.showLeaderboard('score');}});MenuItemAppStore=MenuItem.extend({getText:function(){return'download as app';},ok:function(){window.location.href=ig.ua.iOS?'https://goo.gl/qSntlx':'https://goo.gl/cg7rmI';}});MenuTitle=Menu.extend({itemClasses:[MenuItemNormalMode,MenuItemRemoveAds,MenuItemGameCenter],scale:0.75,y:0,init:function(){this.itemClasses=[];this.itemClasses.push(MenuItemNormalMode);if(window.Ejecta&&!localStorage.getItem('removeAds')){this.itemClasses.push(MenuItemRemoveAds);}
...
if(!ig.ua.mobile){this.itemClasses.push(MenuItemPauseMenu);}
...
this.width=ig.system.width/this.scale;this.playerPos.x=(ig.system.width-this.ship.width)/2;this.playerPos.y=400;},scroll:0,background:new ig.Image('media/background/stars.jpg'),ztype:new ig.Image('media/title/ztype.png'),ship:new ig.Image('media/title/ship.png'),exhaust:new ig.Image('media/title/exhaust.png'),infoIcon:new ig.Image('media/ui/information.png'),glitchStripe:new ig.Image('media/title/glitch-stripe.png'),glitchCode:new ig.Image('media/title/glitch-code.png'),glitchJPEG:new ig.Image('media/title/glitch-jpeg.png'),glitchGraph:new ig.Image('media/title/glitch-graph.png'),glitchLog:new ig.Image('media/title/glitch-log.png'),playerPos:{x:200,y:400},transition:0,draw:function(){var ctx=ig.system.context;ctx.globalAlpha=this.alpha+Math.sin(this.transition*Math.PI)*(1-Math.random()*0.5);ctx.save();ctx.scale(this.scale,this.scale);this.scroll+=ig.system.tick*10;this.scroll=this.scroll%this.background.height;this.background.draw(0,this.scroll-this.background.height);this.background.draw(0,this.scroll);this.ztype.draw((ig.system.width/this.scale-this.ztype.width)/2,150-ig.ease.inOutQuad(this.transition,0,1000,1));ctx.globalAlpha=Math.sin(this.transition*Math.PI);ctx.globalCompositeOperation='lighter';this.glitchStripe.draw(0,4000-this.transition*4200);ctx.restore();var i0y=Math.min(740,ig.system.height/this.scale-210);this.items[0].y=ig.ease.inOutQuad(this.transition,i0y,1600,1);if(this.items.length>1){this.items[1].y=ig.ease.inOutQuad(this.transition,ig.system.height/this.scale-160,1900,1);}
...
ig.baked=true;ig.module('game.keyboard').requires('impact.font').defines(function(){ig.Keyboard=ig.Class.extend({image:new ig.Image('media/ui/keyboard.png'),empButton:new ig.Image('media/ui/emp-buttons.png'),pauseButton:new ig.Image('media/ui/pause.png'),font:new ig.Font('media/fonts/avenir-36-blue.png'),hover:{leftEdge:new ig.Image('media/ui/key-edge-left.png'),rightEdge:new ig.Image('media/ui/key-edge-right.png'),normal:new ig.Image('media/ui/key.png')},multiBar:new ig.Image('media/ui/bar-blue.png'),multiIndicator:new ig.Image('media/ui/multi-indicator.png'),multiSounds:{2:new ig.Sound('media/sounds/multi-2.ogg'),3:new ig.Sound('media/sounds/multi-3.ogg')},width:0,height:0,hoverKey:null,expectedKeys:null,keys:[['q','w','e','r','t','y','u','i','o','p'],['a','s','d','f','g','h','j','k','l'],['z','x','c','v','b','n','m']],layout:{key:{width:64,height:106}},init:function(callback){ig.system.canvas.addEventListener('touchmove',this.touchmove.bind(this),false);ig.system.canvas.addEventListener('touchend',this.touchend.bind(this),false);ig.system.canvas.addEventListener('touchstart',this.touchstart.bind(this),false);this.callback=callback;this.height=ig.ua.mobile?(this.image.height/this.image.width)*ig.system.width:64;this.width=ig.system.width;},getCurrentKey:function(touches){var touch=touches[touches.length-1];var internalWidth=parseInt(ig.system.canvas.offsetWidth)||ig.system.realWidth;var scale=internalWidth/this.width;var keyboardScale=this.image.width/this.width;if(touch.clientX<64&&touch.clientY<64){return'PAUSE';}
var touchX=(touch.clientX/scale)*keyboardScale,touchY=((touch.clientY/scale)-(ig.system.height-this.height))*keyboardScale;if(touchY<-20){return'ESC';}
if(touchY>214&&touchX>560){return'ENTER';}
var row=(touchY/this.layout.key.height)|0;if(row<0||!this.keys[row]){return;}
var offsetX=(this.image.width-this.keys[row].length*this.layout.key.width)/2;var col=((touchX-offsetX)/this.layout.key.width)|0;var key=this.keys[row][col];if(this.expectedKeys){var closest=Infinity;for(var i=0;i<this.expectedKeys.length;i++){var kpos=ig.Keyboard.Map[this.expectedKeys[i]];var keyX=kpos[0]*this.layout.key.width+this.layout.key.width/2,keyY=kpos[1]*this.layout.key.height+this.layout.key.height/2;var dx=touchX-keyX,dy=touchY-keyY;var distance=Math.sqrt(dx*dx+dy*dy);if(distance<this.layout.key.width&&distance<closest){closest=distance;key=this.expectedKeys[i];}}}
return key;},touchstart:function(ev){var key=this.getCurrentKey(ev.touches);if(key!='ESC'&&key!='ENTER'&&key!='PAUSE'){this.hoverKey=key;}},touchmove:function(ev){var key=this.getCurrentKey(ev.touches);if(key!='ESC'&&key!='ENTER'&&key!='PAUSE'){this.hoverKey=key;}},touchend:function(ev){var key=this.getCurrentKey(ev.changedTouches);if(key){this.callback(key);}
this.hoverKey=null;},showMultiplier:function(m){this.multiplierTimer=new ig.Timer(2);this.multiplierIndex=m-2;this.multiSounds[m].play();},drawFull:function(){ig.system.context.globalAlpha=1;ig.system.context.drawImage(this.image.data,0,ig.system.height-this.height,this.width,this.height);var keyboardScale=this.width/this.image.width;var multiLength=1-(75/(75+ig.game.streak*2));ig.system.context.drawImage(this.multiBar.data,0,ig.system.height-this.height,this.width*multiLength,2*keyboardScale);if(this.hoverKey){var ox=0;var img=this.hover.normal;if(this.hoverKey=='q'){img=this.hover.leftEdge;ox=26;}
else if(this.hoverKey=='p'){img=this.hover.rightEdge;ox=-26;}
var kpos=ig.Keyboard.Map[this.hoverKey];var x=(kpos[0]*this.layout.key.width-19+ox)*keyboardScale
y=ig.system.height-this.height+((kpos[1]*this.layout.key.height)-112)*keyboardScale;ig.system.context.globalAlpha=0.9;ig.system.context.drawImage(img.data,x,y,img.width*keyboardScale,img.height*keyboardScale);ig.system.context.globalAlpha=1;this.font.draw(this.hoverKey.toUpperCase(),x+51*keyboardScale,y+24*keyboardScale,ig.Font.ALIGN.CENTER);}
if(this.multiplierTimer){var dt=this.multiplierTimer.delta();ig.system.context.globalAlpha=dt.map(-2,0,2,0).limit(0,1);ig.system.context.drawImage(this.multiIndicator.data,32*this.multiplierIndex,0,32,18,this.width*multiLength-30*keyboardScale,ig.system.height-this.height-(22*keyboardScale)|0,32*keyboardScale,18*keyboardScale);if(dt>=0){this.multiplierTimer=null;}
ig.system.context.globalAlpha=1;}
if(!ig.game.emps){ig.system.context.globalAlpha=0.7;}
var
bx=0,by=ig.system.height-this.height;ig.system.context.drawImage(this.empButton.data,34*(ig.game.emps),0,34,40,bx+582*keyboardScale,by+250*keyboardScale,34*keyboardScale,40*keyboardScale);ig.system.context.globalAlpha=0.15;ig.system.context.drawImage(this.pauseButton.data,28,28,18*keyboardScale,21*keyboardScale);ig.system.context.globalAlpha=1;},drawMinimal:function(){ig.system.context.globalAlpha=1;var keyboardScale=this.width/this.image.width;var multiLength=1-(75/(75+ig.game.streak*2));ig.system.context.drawImage(this.multiBar.data,0,ig.system.height-2*keyboardScale,this.width*multiLength,2);if(this.multiplierTimer){var dt=this.multiplierTimer.delta();ig.system.context.globalAlpha=dt.map(-2,0,2,0).limit(0,1);ig.system.context.drawImage(this.multiIndicator.data,32*this.multiplierIndex,0,32,18,this.width*multiLength-30*keyboardScale,ig.system.height-24,32*keyboardScale,18*keyboardScale);if(dt>=0){this.multiplierTimer=null;}
ig.system.context.globalAlpha=1;}
if(!ig.game.emps){ig.system.context.globalAlpha=0.7;}
var
bx=0,by=ig.system.height-234;ig.system.context.drawImage(this.empButton.data,34*(ig.game.emps),0,34,40,bx+582*keyboardScale,by+250*keyboardScale,34*keyboardScale,40*keyboardScale);},draw:function(){if(ig.ua.mobile){this.drawFull();}
...
if(letter=='PAUSE'){this.menu=new MenuPause();return true;}
...
if(this.gameTransitionTimer){var dt=2-(this.gameTransitionTimer.delta()*-1);this.menu.transition=(dt/2);var move=(ig.system.height-this.keyboard.height-30)-MenuTitle.prototype.playerPos.y;this.menu.playerPos.y=ig.ease.inOutBack(dt,MenuTitle.prototype.playerPos.y,move,2);this.menu.alpha=1-(dt/2);this.player.pos.y=this.menu.playerPos.y;if(dt>2){this.gameTransitionTimer=null;this.menu=null;}}}
...
if(!window.Ejecta&&!window.device){ig.$('body')[0].className=ig.ua.mobile?'mobile':'desktop';var closeButton=ig.$('#gsense-close');if(closeButton&&ig.ua.mobile){setTimeout(function(){closeButton.style.display='block';closeButton.addEventListener('click',function(){ig.$('#gsense').style.display='none';});},1000);}}});